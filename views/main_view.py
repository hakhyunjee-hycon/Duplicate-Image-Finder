...
class MainView(QMainWindow):
    """
    중복 이미지 관리 메인 윈도우 UI (체크박스 멀티셀렉트, 가변 컬럼 폭, 우측 소형 미리보기 지원)
    """
    ...
    def _init_ui(self):
        ...
        # 3-1. 좌측: 중복 이미지 Tree (가변 컬럼 폭 & 체크박스)
        ...
        # 3-2. 우측: 소형 미리보기 패널 (Preview Widget)
        preview_box = QGroupBox("선택 이미지 미리보기")
        ...
        self.lbl_selected_count = QLabel("선택된 항목(체크): 0개")
        self.lbl_selected_count.setFont(QFont("Malgun Gothic", 9, QFont.Weight.Bold))
    ...
    def _update_checked_count(self):
        count = len(self.get_checked_file_paths())
        self.lbl_selected_count.setText(f"선택된 항목(체크): {count}개")
    ...
    def _clear_preview(self):
        ...
        self.lbl_preview_img.setText("선택된 이미지 없음")
    ...
    def _update_preview_panel(self, file_path: str):
        ...
        try:
            ...
            # 소형 미리보기 썸네일 스케일링
            target_size = self.lbl_preview_img.size()
    ...
    def _on_delete_clicked(self):
        ...
        reply = QMessageBox.question(
            self, "확인", f"체크 선택한 {len(file_paths)}개 파일을 정말 삭제하시겠습니까?",
